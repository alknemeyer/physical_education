...
class System2D:
    def __init__(self, name: str, links: List['Link2D']) -> None:
        self.name = name
        self.links = links
        self.m: Union[ConcreteModel, None] = None
        self.sp_variables: List[sp.Symbol] = flatten(link.get_sympy_vars() for link in self.links)

    def add_link(self, link: 'Link2D') -> None:
        self.links.append(link)

    def get_state_vars(self) -> Tuple[Mat, Mat, Mat]:
        q = getattrs(self.links, 'q')
        dq = getattrs(self.links, 'dq')
        ddq = getattrs(self.links, 'ddq')
    ...
    def calc_eom(self, *, simp_func: Callable[[T], T] = lambda x: x) -> None:
        ...
        positions = getattrs(self.links, 'Pb_I')
        rotations = getattrs(self.links, 'Rb_I')
        masses = getattrs(self.links, 'mass_sym')
        inertias = getattrs(self.links, 'inertia')
        ...
        ang_vels = [
            Mat([
                utils.skew_symmetric(rot_2d_to_3d(Rx_I), utils.vec(*q, sp.symbols("z")),
                                     utils.vec(*dq, sp.symbols("dz")))[2]
            ]) for Rx_I in rotations
        ]
        ...
        Ep = Mat([sum(m * Mat([0, 9.81]).dot(p) for (m, p) in zip(masses, positions))])
        ...
        eom = M @ ddq + C + G - B
        eom = simp_func(Mat([*eom]).xreplace(to_sub))
    ...
    def init_from_dict_one_point(self,
        ...
        if not m.hm0.value == data['hm0']:
            visual.warn(f'init_from_dict_one_point: self.hm0 = {m.hm0.value} != {data["hm0"]} = data["hm0"]', once=True)
    ...
    def __getitem__(self, linkname: str) -> 'Link2D':
        for link in self.links:
    ...
    def animate(self,
                t_scale: float = 1.,
                plot2d_config: Dict = {},
                lims: Optional[Tuple[Tuple, Tuple]] = None,
        ...
        fig, ax = visual.plot2d_setup(scale_plot_size=False, **plot2d_config)
        ...
        if dt is None:
            ...
            def _animate(fe: int):  # fe is one-based
                ...
                for link in self.links:
                    link.animation_update(fig, ax, fe=fe)
        else:
            ...
            def _animate(t: float):  # t is a float from 0 to total_time
                ...
                for link in self.links:
                    link.animation_update(fig, ax, t=t, t_arr=t_arr)
        ...
        try:
            ...
            anim = matplotlib.animation.FuncAnimation(fig, _animate, frames=frames, interval=interval_ms)
    ...
    def __repr__(self) -> str:
        ...
        return f'System2D(name="{self.name}", [\n  {child_links}\n])'
...
class System3D:
    ...
    def calc_eom(self, *, simp_func: Callable[[T], T] = lambda x: x) -> None:
        ...
        Ek, Ep, _, _ = utils.calc_velocities_and_energies(getattrs(self.links, 'Pb_I'),
        ...
        angle_constraints = simp_func(Mat(flatten(link.angle_constraints for link in self.links)))
