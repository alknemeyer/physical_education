...
def _printutil(prefix: str, s: str, colour: str, once: bool):
    ...
    if USE_COLOURS is True:
        print(bcolors.BOLD + colour + str(prefix) + bcolors.ENDC, colour + calling_func + str(s) + bcolors.ENDC)
    else:
...
def plot3d_setup(figsize=(10, 10),
    ...
    def add_ground(ground_lims: Tuple[Tuple[float, float], Tuple[float, float]],
        ...
        xx, yy = ground_lims
        ground = ax.plot_surface(*np.meshgrid(xx, yy), np.zeros((2, 2)), alpha=alpha, color=color, zorder=1)
        return ground

    if scale_plot_size is True:
        fig.subplots_adjust(left=-0.25, bottom=-0.25, right=1.25, top=1.25, wspace=None, hspace=None)

    return fig, ax, add_ground


def plot2d_setup(figsize=(10, 10),
                 dpi: int = 60,
                 title: Optional[str] = None,
                 show_grid: bool = True,
                 xyz_labels: Optional[Tuple[str, str, str]] = None,
                 scale_plot_size: bool = True):
    """Set up a fairly "standard" figure for a 2D system, including:
    - setting up the title correctly
    - setting limits on and labelling axes
    - optionally removing the grid
    - scaling the plot to reduce surrounding whitespace
...
def update_2d_line(line, pt1: List[float], pt2: List[float]):
    """Update data in a 2D line, passed as two points"""
    line.set_data([[pt1[0], pt2[0]], [pt1[1], pt2[1]]])
...
class LineAnimation:
    def __init__(self,
                 ...
                 dim: Literal[3, 2] = 3,
                 is_fixed_base: bool = False) -> None:
        assert pos1.shape == (dim, 1)
        assert pos2.shape == (dim, 1)

        self.dim = dim
        self.is_fixed_base = is_fixed_base
        from .utils import lambdify_EOM
        if is_fixed_base:
            self.pos1func = [lambda *sp_variables: 0, lambda *sp_variables: 0]
        else:
    ...
    def animation_setup(self, fig, ax, data: List[List[float]]):
        self.line = ax.plot([], [], [], linewidth=1, color='darkgray')[0]

        self.plot_data = [np.empty((len(data), self.dim)), np.empty((len(data), self.dim))]
    ...
    def animation_update(self,
        ...
        if fe is not None:
            ...
            pos1_xyz = [np.interp(t, t_arr, self.plot_data[0][:, i]) for i in range(self.dim)]
            pos2_xyz = [np.interp(t, t_arr, self.plot_data[1][:, i]) for i in range(self.dim)]

        if self.dim == 3:
            update_3d_line(self.line, pos1_xyz, pos2_xyz)
        else:
            update_2d_line(self.line, pos1_xyz, pos2_xyz)

        if track is True and self.dim == 3:
            lim = 1.0
            if pos1_xyz[2] < lim:
...
def data_for_3d_cylinder(top, bottom, radius: float, nsides: int):
    ...
    x_grid = np.array([bx + sin(z2pi) * radius, tx + sin(z2pi) * radius])
    y_grid = np.array([by + cos(z2pi) * radius, ty + cos(z2pi) * radius])
    ...
    n = top - bottom
    b = np.array([0, 0, 1])
    th_to_xy_plane_rad = arcsin(b.dot(n) / np.linalg.norm(b) / np.linalg.norm(n))
    z_grid = np.repeat(np.array([bz, tz]), x_grid.shape[1]).reshape(x_grid.shape)
...
class CylinderAnimation:
    ...
    def animation_setup(self, fig, ax, data: List[List[float]]):
        self.plot_data = [np.empty((len(data), 3)), np.empty((len(data), 3))]
    ...
    def animation_update(self,
        ...
        if fe is not None:
            ...
            pos1_xyz = [np.interp(t, t_arr, self.plot_data[0][:, i]) for i in range(3)]
            pos2_xyz = [np.interp(t, t_arr, self.plot_data[1][:, i]) for i in range(3)]

        if hasattr(self, 'line'):
            self.line.remove()
            del self.line

        Xc, Yc, Zc = data_for_3d_cylinder(pos1_xyz, pos2_xyz, radius=self.radius, nsides=self.nsides)
...
def stitch_together_animation(robot: 'System3D', data: List[dict]):
    ...
    robot.make_pyomo_model(nfe=nfe,
                           collocation='implicit_euler',
                           total_time=total_time,
                           vary_timestep_within=(0.5, 1.5),
    ...
    for idx, d in enumerate(data):
        ...
        for fes in range(dnfe):
            robot.init_from_dict_one_point(d,
                                           fed=nfe_otg + fes,
                                           cpd=1,
                                           fes=fes,
                                           cps=0,
        ...
        if idx != 0:
            ...
            def var(fe, coord):
                return bodyq[nfe_otg + fe, 1, coord]
