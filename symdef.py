import sympy as sp
from sympy import Matrix as Mat  # TODO: find out why typing fails for this
...
def make_xyz_syms(i: str) -> Tuple[Mat, Mat, Mat]:
    """
    Define x, y, z symbols and time derivatives
    ...
    if USE_LATEX is True:
        ...
        ddq = sp.symbols(r'\ddot{x}_{%s} \ddot{y}_{%s} \ddot{z}_{%s}' % (i,i,i))
    else:
...
def make_ang_syms(i: str) -> Tuple[Mat, Mat, Mat]:
    ...
    if USE_LATEX is True:
        ...
        dq = sp.symbols(r'\dot{\phi}_{%s} \dot{\theta}_{%s} \dot{\psi}_{%s}' % (i,i,i))
        ddq = sp.symbols(r'\ddot{\phi}_{%s} \ddot{\theta}_{%s} \ddot{\psi}_{%s}' % (i,i,i))
    else:
