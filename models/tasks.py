...
def drop_test(robot, *, z_rot: float, min_torque: bool, initial_height: float = 1.) -> Dict[str,Any]:
    ...
    tested_models = ('3D monoped', '3D biped', '3D quadruped', '3D prismatic monoped')
    if not robot.name in tested_models:
        utils.warn('This robot configuration hasn\'t been tested!')
    ...
    for upper, lower in uplopairs:
        ang = 0.01 if not (robot.name == '3D quadruped' and upper[1] == 'B') else -0.01
    ...
    # TODO: replace bit below with add_costs() !
    # objective: reduce CoT, etc
    if hasattr(robot.m, 'cost'):
        utils.debug('Deleting previous cost function')
